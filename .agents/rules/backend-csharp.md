...
- **`DeleteBehavior.Cascade`/`.SetNull`/`.Restrict` NÃO são proibidos** — são usados extensivamente no schema físico (ver `backend/docs/database-iot.md`). O que É esperado: desvinculação manual de FKs opcionais em memória no Handler antes do soft-delete (ex: `DeleteRoomCommandHandler` zera `device.RoomId` num loop), já que o soft-delete nunca aciona a constraint física.
