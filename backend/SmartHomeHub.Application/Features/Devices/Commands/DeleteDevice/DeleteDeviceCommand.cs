...
public class DeleteDeviceCommandHandler(IAppDbContext dbContext) : ICommandHandler<DeleteDeviceCommand, Result>
{
    public async ValueTask<Result> Handle(DeleteDeviceCommand request, CancellationToken cancellationToken)
    {
        ...
        if (device == null)
            return Result.Failure(new Error("Device.NotFound", "Dispositivo não encontrado ou sem permissão para exclusão."));
