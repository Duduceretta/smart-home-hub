...
7. **Sweep periódico incondicional via worker de polling (~12s)**: para evitar vazamento de descritores de sockets TCP e sessões órfãs em dispositivos soft-deleted (ou abandonados sem novos comandos), o `TuyaDeviceStatePollingWorker` invoca `PruneExpiredSessions()` incondicionalmente a cada tick de 12s, além da checagem reativa que já ocorria em `ExecuteAsync`. A operação é puramente em memória e idempotente (no-op se nenhuma sessão expirou).
8. **Invalidação imediata de sessão em falha criptográfica (`CryptographicException`)**: se uma operação sobre uma sessão em cache falhar por erro criptográfico (HMAC divergente em v3.4 ou falha de autenticação de tag AES-GCM em v3.5 — sintoma clássico de dessincronização de session key ou sequence number), a sessão suspeita é descartada e fechada imediatamente do cache `_sessions`. A exceção é re-lançada para retornar `Device.InvalidLocalKey` ao chamador, garantindo que qualquer tentativa subsequente force um novo handshake de 3 vias limpo em vez de reutilizar a sessão corrompida.
