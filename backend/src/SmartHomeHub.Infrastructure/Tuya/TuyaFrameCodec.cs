...
//
// Puro: construção/parsing de frames, sem I/O — extraído de
// TuyaSessionProtocolClient (ver backend-audit-2026-09-05.md, seção 05).
// Extração mecânica, mesma lógica; cache de sessão, ciclo de vida de conexão e
// reconexão continuam em TuyaSessionProtocolClient, que chama estes métodos
// pra montar/parsear os frames que envia/recebe.
public static class TuyaFrameCodec
{
    ...
    // Comando REAL usado no wire pra escrita, confirmado capturando tráfego
    // real do tinytuya (v3.5) em 2026-08-30 — exposto como public pois
    // TuyaSessionProtocolClient.SetDpsAsync também precisa dele pra chamar
    // ExecuteAsync com o código de comando certo (o dispositivo real
    // ignora/derruba a conexão se receber o código "clássico" CmdControl acima).
    public const int CmdControlNew = 0x0D; // 13

    // Prefix55Aa/Prefix6699 são public: TuyaSessionProtocolClient.ReceiveFrameAsync
    // também precisa reconhecê-los pra decidir o layout de header ao reassemblar
    // um frame recebido do socket, antes mesmo de chamar qualquer método de
    // parsing deste codec.
    public const uint Prefix55Aa = 0x000055AA;
    private const uint Suffix55Aa = 0x0000AA55;
    public const uint Prefix6699 = 0x00006699;
    ...
    public static IReadOnlyDictionary<int, object?> ParseCommandResponse(
        bool useGcm,
        byte[] sessionKey,
        byte[] responseFrame
    )
