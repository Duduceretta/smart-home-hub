...
import { useDeviceAutomations } from "../../hooks/useDeviceAutomations";
import type { DeviceAutomationTriggerKind } from "../../types/devices.types";

interface DeviceLinkedAutomationsProps {
	deviceId: string;
}

const TRIGGER_ICON: Record<DeviceAutomationTriggerKind, typeof Clock> = {
	schedule: Clock,
	sensor: Radio,
	unknown: Bot,
};

const TRIGGER_LABEL_KEY: Record<DeviceAutomationTriggerKind, string> = {
	schedule: "automations.triggerSchedule",
	sensor: "automations.triggerSensor",
	unknown: "automations.triggerUnknown",
};

const TRIGGER_LABEL_FALLBACK: Record<DeviceAutomationTriggerKind, string> = {
...
/**
 * Automações que referenciam este dispositivo — cruzamento feito no
 * back-end (`GET /devices/{id}/automations`, ver GetDeviceAutomationsQuery.cs).
 * Cópia adaptada de `rooms/components/detail/RoomLinkedAutomations.tsx`
 * (isolamento do FSD). Clique navega pra `/automations` — mesma limitação
 * já documentada lá: a tela de Automações não suporta deep-link por
 * seleção ainda.
 */
export function DeviceLinkedAutomations({
	deviceId,
}: DeviceLinkedAutomationsProps) {
	...
	const {
		...
		isError,
		refetch,
	} = useDeviceAutomations(deviceId);

	return (
		<div className="flex flex-col gap-2">
			<div className="flex items-center justify-between">
				<h3 className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
					{t("automations.title", "Automações que usam este dispositivo")}
				</h3>
			</div>
			...
			{isLoading ? (
				...
				<div className="rounded-lg border border-dashed border-border-subtle bg-surface-container/20 p-4 text-center">
					<p className="text-xs text-muted-foreground">
						{t(
							"automations.empty",
							"Nenhuma automação configurada para este dispositivo.",
						)}
					</p>
					<button
						type="button"
